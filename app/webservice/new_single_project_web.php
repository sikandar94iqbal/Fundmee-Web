...
if($res_rating){
  ...
  if($cnt_rating == 0){
    ...
    if($res_rating_sql){
      while($row = mysqli_fetch_assoc($res_rating_sql)){
        $ratings = $row['rating'];
        $rating_data_array[] = $row;
      }
    }
  }
...
$std->project_data = $project_data_array;
$std->project_backers_data = $backers_data_array;
$std->project_rating = $rating_data_array;
